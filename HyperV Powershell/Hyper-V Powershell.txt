...
Foreach ($VM in $VMs)
{
    ...
    [float]$VHD_Used = ("{0:N2}" -f ((($driveArrMax.Get_Item($driveFor.Key))/1gb -as [float])/($HostDriveTotal/1gb -as [float]) * 100))
...
#Clear the VHD ForEach string variables
$vhd_table = ""
$vhd_usedPB = ""
}
